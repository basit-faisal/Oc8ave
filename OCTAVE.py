...
def Saver_1():
    ...
    # Find the next available row by locating the first empty row in column A
    next_row = 2
...
def step():  # just a widget with no effect but an infinity progress bar
    PB.start(10)
...
Label(root, text='Preferred Portfolio', font="Sans", padx=10, bg='gray').grid(row=4, column=0, pady=15)
...
first_name_input.bind('<Return>', go_to_next_entry_lastname)  # connecting enter key with going to the next text box <binding>
